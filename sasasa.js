...
function sendMessage() {
  ...
  if (userMessage !== '') {
    ...
    if (containsNSFW(userMessage)) {
      ...
      // If the user explicitly asks for Lina's photo, send the specific photo URL
      const linaPhotoUrl = 'https://i.ibb.co/f1GN6SQ/photo-2023-08-16-15-20-24.jpg';
      displayMessage('Lina', `<img src="${linaPhotoUrl}" alt="Lina's Photo" style="max-width: 100%;">`, 'bot-message');
    } else if (userMessage.includes('send me photo of')) {
      // Extract the subject the user wants a photo of
      const subjectIndex = userMessage.indexOf('send me photo of') + 'send me photo of'.length;
      const subject = userMessage.substring(subjectIndex).trim();

      // Build the URL for the photo based on the user's request
      const apiUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(subject)}?width=640&height=640`;

      // Display the photo in the message bubble
...
function displayMessage(sender, message, className) {
  ...
  // Create like, dislike, and copy buttons for Lina's messages
  if (sender === 'Lina') {
...
function sendToTelegram(message) {
  if (user) {
    ...
    // Include user details in the message
    const userMessage = `Name: ${user.name}\nBirthdate: ${user.birthdate}\n\n${message}`;
